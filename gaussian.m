...
gau=imnoise(anhThuyVan,'gaussian', 0.01); 
axes(handles.axes2);         %Fisso l'axes per il plot
imshow(gau);


cr = max(max(normxcorr2(anhGoc(:,:,1), gau(:,:,1))));
cg = max(max(normxcorr2(anhGoc(:,:,2), gau(:,:,2))));
cb = max(max(normxcorr2(anhGoc(:,:,3), gau(:,:,3))));
c=(cr+cg+cb)/3;
%figure, surf(c), shading flat
set(handles.txtncc1,'String',c);

% PSNR
p = PSNR_RGB(anhGoc,gau);
...
% Muoi Tieu
anhThuyVan=handles.anhThuyVan;
gau=imnoise(anhThuyVan,'gaussian', 0.03); 
axes(handles.axes2);         %Fisso l'axes per il plot
imshow(gau);
...
% Muoi Tieu
anhThuyVan=handles.anhThuyVan;
gau=imnoise(anhThuyVan,'gaussian', 0.02); 
axes(handles.axes2);         %Fisso l'axes per il plot
imshow(gau);
